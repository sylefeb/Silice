...
$$end
  ...
  bram uint16 bsp_ssecs_flats[] = {
$$for _,s in ipairs(bspSSectors) do
   $pack_bsp_ssec_flats(s)$,          // c_T=$s.c_T$ f_T=$s.f_T$
$$end
  }; 
...
++: // relax timing                  
                  ...
                  () <- writePixel <- (c,btm,(tr_gu_m>>4),(tr_gv_m>>4),texid);
                  btm = btm + 1;
                  inv_y.addr = 100 - btm;
                }
...
++: // relax timing                  
                  ...
                  () <- writePixel <- (c,top,(tr_gu_m>>4),(tr_gv_m>>4),texid);
                  top = top - 1;
                  inv_y.addr = top - 100;
                }
...
++:
                  ...
                  while (j > btm) {
                    ...
                    () <- writePixel <- (c,j,tc_u,bsp_segs_texmapping.rdata[32,16]+tc_v,texid);
